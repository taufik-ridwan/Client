...
public class Packet extends ElytraFlightMode {
    ...
    public void onTick() {
        ...
        if (mc.options.keyJump.isPressed()) vec3d.add(0, elytraFly.verticalSpeed.get(), 0);
        else if (!mc.options.keyJump.isPressed()) vec3d.add(0, -elytraFly.verticalSpeed.get(), 0);
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (event.packet instanceof PlayerMoveC2SPacket) mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
    }

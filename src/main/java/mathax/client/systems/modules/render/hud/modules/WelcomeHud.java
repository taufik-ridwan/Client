...
import mathax.client.systems.modules.render.hud.TripleTextHudElement;
    ...
    protected String getLeft() {
        switch (message.get()) {
            case Using -> {
                ...
                if (MatHax.isDeveloper(mc.getSession().getUuid())) return "You are using MatHax, Developer ";
                else return "You are using MatHax, ";
            }
            case Time -> {
                if (Modules.get().isActive(NameProtect.class)) return getTime() + ", ";
                if (MatHax.isDeveloper(mc.getSession().getUuid())) return getTime() + ", Developer ";
                else return getTime() + ", ";
            }
            case Retarded_Time -> {
                if (Modules.get().isActive(NameProtect.class)) return getRetardedTime() + ", ";
                if (MatHax.isDeveloper(mc.getSession().getUuid())) return getRetardedTime() + ", Developer ";
                else return getRetardedTime() + ", ";
            }
            case Sussy -> {
                if (Modules.get().isActive(NameProtect.class)) return "You are a sussy baka, ";
                if (MatHax.isDeveloper(mc.getSession().getUuid())) return "You are a sussy baka, Developer ";
                else return "You are a sussy baka, ";
            }
            default -> {
                if (Modules.get().isActive(NameProtect.class)) return "Welcome to MatHax, ";
                if (MatHax.isDeveloper(mc.getSession().getUuid())) return "Welcome to MatHax, Developer ";

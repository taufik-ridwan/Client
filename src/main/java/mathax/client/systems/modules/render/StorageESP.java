...
import mathax.client.utils.render.MeshVertexConsumerProvider;
import mathax.client.utils.render.RenderUtils;
import mathax.client.utils.render.SimpleBlockRenderer;
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        for (BlockEntity blockEntity : Utils.blockEntities()) {
            ...
            if (render) {
                ...
                if (tracers.get() && a >= 0.075) event.renderer.line(RenderUtils.center.x, RenderUtils.center.y, RenderUtils.center.z, blockEntity.getPos().getX() + 0.5, blockEntity.getPos().getY() + 0.5, blockEntity.getPos().getZ() + 0.5, lineColor);
        ...
        if (mode.get() == Mode.Shader) EntityShaders.renderOutlines(() -> mesh.render(event.matrices), false, outlineWidth.get(), fillOpacity.get(), shapeMode.get());
    }
